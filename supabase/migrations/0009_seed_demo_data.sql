...
--
-- BEFORE RUNNING: replace the patient email placeholder below.
--   patient_email — your real login email (stays role = 'patient' — kept separate from
--                   doctor/admin so role-gated features added later don't get confused by a
--                   single account wearing multiple hats).
--   doctor_email / admin_email — two dedicated demo accounts, each created via Supabase →
--                   Authentication → Users → Add user with Auto Confirm User checked (no OTP or
--                   real inbox needed). This script promotes them to 'doctor' and 'admin' and
--                   uses them to own the demo appointments/calls and the published content.
